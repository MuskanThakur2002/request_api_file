...
while i>0:
    for j in range(len(data["availableCourses"])):
        print(j+1,":-", data["availableCourses"][j]['name'],data["availableCourses"][j]['id'])
        a.append(data["availableCourses"][j]['id'])
    id=int(input("Select any course:- "))
    s1= "http://saral.navgurukul.org/api/courses/"+a[id-1]+"/exercises" 
    s2=requests.get(s1)
    s3=s2.json()
    x=[]
    for k in range(len(s3['data'])):
        print(k+1,":-", s3['data'][k]['name'], s3['data'][k]['parent_exercise_id'])
    ...
    select_continue=input("you want to continue with (yes/no)")
    if select_continue=="yes":
        ...
        for k in s3['data']:
            if k['id']==x[id1-1]:
                ...
                for i in range(len(k['childExercises'])):
                    print("         ",i+1,":-", k['childExercises'][i]['id'], k['childExercises'][i]['name'])
                    r.append(k['childExercises'][i]['id'])
                select=input("you want to continue with (yes/no)")
                if select=="yes":
